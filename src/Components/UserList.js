...
function UserList() {
    ...
    useEffect(() => {
      Axios.get(API)
        ...
        .catch((err) => console.log("ERR: FETCHING DATA FAILED", err));
    }, []);
    ...
    const handleAddUser = () => {
      Axios.post(API, {
        name: "Ganesamoorthy",
        phone: "1234567890",
        email: "gurunathganesh@sifycorp.com",
        username: "Right"
      })
        .then((result) => {
    ...
    return (
      <div>
        <div className="d-flex justify-content-end mb-3">
          <Button variant="Success" onClick={handleAddUser} className="btn btn-success">
            Add New User
          </Button>
        </div>
        <Table bordered>
